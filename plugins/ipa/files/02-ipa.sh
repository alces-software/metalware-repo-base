...
ipa-client-install --no-ntp --mkhomedir --no-ssh --no-sshd --force-join --realm="$REALM" --server="<%= node.plugins.ipa.config.ipa_servername %>.<%= config.networks.pri.domain %>.<%= config.domain %>" -w "<%= node.plugins.ipa.config.insecurepassword %>" --domain="<%= config.networks.pri.domain %>.<%=config.domain %>" --unattended --hostname='<%= config.networks.pri.hostname %>'
EOF
<% end -%>
