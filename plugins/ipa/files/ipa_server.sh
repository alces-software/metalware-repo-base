...
ipa config-mod --homedirectory <%= node.plugins.ipa.config.ipa_userdir %>
...
<%     group.nodes.each do |anode| -%>
ipa host-add <%= anode.config.networks.pri.hostname %> --password="<%= node.plugins.ipa.config.ipa_insecurepassword %>" --ip-address=<%= anode.config.networks.pri.ip %>
<%     end -%>
<% end -%>
